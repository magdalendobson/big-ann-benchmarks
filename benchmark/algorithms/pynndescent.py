...
class pynndescent(BaseOODANN):
    def __init__(self, metric, index_params):
        self.name = "pynndescent"
        if (index_params.get("max_deg")==None):
            print("Error: missing parameter max_deg")
            return
        self._index_params = index_params
        self._metric = self.translate_dist_fn(metric)

        self.max_deg = int(index_params.get("max_deg"))
        self.num_clusters = int(index_params.get("num_clusters", 10))
    ...
    def create_index_dir(self, dataset):
        ...
        index_dir = os.path.join(index_dir, 'pynndescent')
        os.makedirs(index_dir, mode=0o777, exist_ok=True)
    ...
    def fit(self, dataset):
        ...
        if hasattr(self, 'index'):
            ...
            pann.build_pynndescent_index(self._metric, self.translate_dtype(ds.dtype), ds.get_dataset_fn(), index_dir, self.max_deg, self.num_clusters, self.cluster_size, self.alpha, self.delta)
            end = time.time()
